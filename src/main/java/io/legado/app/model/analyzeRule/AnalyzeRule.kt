...
class AnalyzeRule(
    ...
    fun reGetBook() {
        ...
        runBlocking {
            withTimeout(1800000) {
                WebBook(bookSource, userNameSpace = getUserNameSpace()).preciseSearch(book.name, book.author)
                    .getOrThrow().let {
                ...
                WebBook(bookSource, userNameSpace = getUserNameSpace()).getBookInfo(book, false)
            }
        }
    }
    ...
    fun refreshBookUrl() {
        runBlocking {
            ...
            val books = WebBook(bookSource, userNameSpace = getUserNameSpace()).searchBook(book.name)
            books.forEach {
    ...
    fun refreshTocUrl() {
        runBlocking {
            ...
            val book = book as? Book
            if (bookSource == null || book == null) return@runBlocking
            WebBook(bookSource, userNameSpace = getUserNameSpace()).getBookInfo(book)
        }
    }
